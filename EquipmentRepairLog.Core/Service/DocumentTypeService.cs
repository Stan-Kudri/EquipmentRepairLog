...
namespace EquipmentRepairLog.Core.Service
{
    public class DocumentTypeService(AppDbContext dbContext, DocumentTypeFactory documentTypeFactory)
    {
        public void Add(DocumentType documentType)
        {
            ...
            {
                throw new EquipmentRepairLogException($"Document Type \"{documentType.Name}\" have already been add to the app (DB).");
            }
        ...
        public void Remove(Guid id)
        {
            var item = dbContext.DocumentTypes.FirstOrDefault(e => e.Id == id)
                        ?? throw new EquipmentRepairLogException($"The ID for the document type  with \"{id}\" is already taken.");
